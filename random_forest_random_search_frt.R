source("random_forest_main_frt_pipeline_functions.R")
...
select_rf_feature_subset <- function(X_df, rf_mtry_ratio) {
  p <- ncol(X_df)
  mtry_count <- max(1, min(p, floor(p * rf_mtry_ratio)))
  sample(names(X_df), size = mtry_count, replace = FALSE)
}
...
fit_single_tree_frt_predict <- function(
  ...
  selected_features <- intersect(selected_features, names(X_fit))
  if (!length(selected_features)) {
    stop("No valid feature subset was selected for the random forest tree.")
  }
  
  X_fit_sub <- X_fit[, selected_features, drop = FALSE]
  X_eval_train_sub <- X_eval_train[, selected_features, drop = FALSE]
  X_eval_test_sub <- X_eval_test[, selected_features, drop = FALSE]
  
  df_fit_model <- data.frame(X_fit_sub, Target = Y_fit)
  ...
  cart_pred_train <- as.numeric(predict(tree0, X_eval_train_sub))
  cart_pred_test <- as.numeric(predict(tree0, X_eval_test_sub))
  
  GFN_X_fit <- lapply(X_fit_sub, fuzzify, fuzzy.var)
  GFN_Y_fit <- fuzzify(Y_fit, fuzzy.var)
  GFN_X_eval_train <- lapply(X_eval_train_sub, fuzzify, fuzzy.var)
  GFN_X_eval_test <- lapply(X_eval_test_sub, fuzzify, fuzzy.var)
  
  routed_train_initial <- route_all(tree0, X_eval_train_sub)
  routed_test_initial <- route_all(tree0, X_eval_test_sub)
...
run_random_forest_frt_configurable <- function(
  ...
  rf_size <- max(2, floor(n_train * rf_sample_frac))
  ...
  flips_per_tree <- numeric(rf_ntree)
  
  for (b in seq_len(rf_ntree)) {
    boot_idx <- sample(seq_len(n_train), size = rf_size, replace = TRUE)
    selected_features <- select_rf_feature_subset(X_train, rf_mtry_ratio)
...
sample_random_config <- function() {
  ...
  list(
    rf_ntree = sample(c(50, 100, 150), 1),
    rf_sample_frac = sample(c(0.75, 1.00), 1),
    rf_mtry_ratio = sample(c(0.35, 0.50, 0.65, 0.80), 1),
...
evaluate_config_on_all_datasets <- function(
  ...
  for (train_file in train_files) {
    ...
    res <- tryCatch(
      do.call(
        run_random_forest_frt_configurable,
        c(
          list(
    ...
    detail_rows[[length(detail_rows) + 1]] <- data.frame(
      ...
      rf_ntree = cfg$rf_ntree,
      rf_sample_frac = cfg$rf_sample_frac,
      rf_mtry_ratio = cfg$rf_mtry_ratio,
      cart_cp = cfg$cart_cp,
      ...
      RF_MSE_train = res$cart$train_err$MSE,
      RF_RMSE_train = res$cart$train_err$RMSE,
      RF_MAE_train = res$cart$train_err$MAE,
      RF_MAPE_train = res$cart$train_err$MAPE,
      RF_MSE_test = res$cart$test_err$MSE,
      RF_RMSE_test = res$cart$test_err$RMSE,
      RF_MAE_test = res$cart$test_err$MAE,
      RF_MAPE_test = res$cart$test_err$MAPE,
      RF_init_FMSE_train = res$cart$fuzzy_train_initial$FMSE,
      RF_init_FMSE_test = res$cart$fuzzy_test_initial$FMSE,
      RF_FRT_final_FMSE_train = res$fuzzy$train_final$FMSE,
      RF_FRT_final_FMSE_test = res$fuzzy$test_final$FMSE,
      RF_FRT_crisp_MSE_train = res$fuzzy$crisp_train_err$MSE,
      RF_FRT_crisp_RMSE_train = res$fuzzy$crisp_train_err$RMSE,
      RF_FRT_crisp_MAE_train = res$fuzzy$crisp_train_err$MAE,
      RF_FRT_crisp_MAPE_train = res$fuzzy$crisp_train_err$MAPE,
      RF_FRT_crisp_MSE_test = res$fuzzy$crisp_test_err$MSE,
      RF_FRT_crisp_RMSE_test = res$fuzzy$crisp_test_err$RMSE,
      RF_FRT_crisp_MAE_test = res$fuzzy$crisp_test_err$MAE,
      RF_FRT_crisp_MAPE_test = res$fuzzy$crisp_test_err$MAPE,
...
random_search_cart_frt <- function(
    ...
    seed = 123,
    verbose = FALSE,
    export_prefix = "random_forest_random_search_rf_frt") {
  ...
  for (iter in seq_len(n_iter)) {
    ...
    if (verbose) {
      cat("\nRandom forest random search iteration:", iter, "of", n_iter, "\n")
    }
