source("main_frt_pipeline_functions.R")
...
run_cart_frt_configurable <- function(
...
random_search_cart_frt <- function(
    ...
    seed = 123,
    verbose = FALSE,
    export_prefix = "random_search_cart_frt") {
  ...
  for (iter in seq_len(n_iter)) {
    ...
    if (verbose) {
      cat("\nRandom search iteration:", iter, "of", n_iter, "\n")
    }
...
random_search_cart_frt_per_dataset <- function(
    n_iter = 50,
    train_dir = "train_data",
    test_dir = "test_data",
    seed = 123,
    verbose = FALSE,
    export_prefix = "random_search_cart_frt_per_dataset") {
  
  set.seed(seed)
  
  train_files <- list.files(train_dir, pattern = "\\.csv$", full.names = TRUE)
  if (!length(train_files)) {
    stop("No train csv files found in train_dir.")
  }
  
  all_summary_rows <- list()
  all_detail_rows <- list()
  best_summary_rows <- list()
  best_detail_rows <- list()
  ...
  for (train_file in train_files) {
    ...
    if (verbose) {
      cat("\nPer-dataset random search:", dataset_name, "\n")
    }
    
    dataset_summary_rows <- list()
    dataset_detail_rows <- list()
    ...
    for (iter in seq_len(n_iter)) {
      ...
      eval_res <- evaluate_config_on_single_dataset(
        cfg = cfg,
      ...
      if (is.null(eval_res$summary) || is.null(eval_res$details)) {
        next
      }
      ...
      dataset_summary_rows[[length(dataset_summary_rows) + 1]] <- eval_res$summary
      dataset_detail_rows[[length(dataset_detail_rows) + 1]] <- eval_res$details
    }
    
    if (!length(dataset_summary_rows)) {
      next
    }
    
    dataset_summary_df <- do.call(rbind, dataset_summary_rows)
    dataset_detail_df <- do.call(rbind, dataset_detail_rows)
    
    dataset_summary_df <- dataset_summary_df[
      order(
        -dataset_summary_df$objective,
        -dataset_summary_df$improved_test_MSE_count,
        -dataset_summary_df$improved_test_FMSE_count
      ),
    ]
    
    best_iteration <- dataset_summary_df$iteration[1]
    best_detail_df <- dataset_detail_df[dataset_detail_df$iteration == best_iteration, ]
    
    all_summary_rows[[length(all_summary_rows) + 1]] <- dataset_summary_df
    all_detail_rows[[length(all_detail_rows) + 1]] <- dataset_detail_df
    best_summary_rows[[length(best_summary_rows) + 1]] <- dataset_summary_df[1, ]
    best_detail_rows[[length(best_detail_rows) + 1]] <- best_detail_df
  }
  
  if (!length(best_summary_rows)) {
    stop("Per-dataset random search produced no valid results.")
  }
  
  all_summary_df <- do.call(rbind, all_summary_rows)
  all_detail_df <- do.call(rbind, all_detail_rows)
  best_summary_df <- do.call(rbind, best_summary_rows)
  best_detail_df <- do.call(rbind, best_detail_rows)
  
  best_summary_df <- best_summary_df[
    order(best_summary_df$dataset),
  ]
  best_detail_df <- best_detail_df[
    order(best_detail_df$dataset),
  ]
  
  openxlsx::write.xlsx(
    list(
      best_summary_per_dataset = best_summary_df,
      best_details_per_dataset = best_detail_df,
      all_iteration_summary = all_summary_df,
      all_iteration_details = all_detail_df
    ),
    file = paste0(export_prefix, ".xlsx"),
    rowNames = FALSE
  )
